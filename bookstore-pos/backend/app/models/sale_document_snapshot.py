...
class SaleDocumentSnapshot(Base):
    ...
    template_id: Mapped[int | None] = mapped_column(ForeignKey("print_templates.id"), nullable=True)
    template_version_id: Mapped[int | None] = mapped_column(ForeignKey("print_template_versions.id"), nullable=True)
