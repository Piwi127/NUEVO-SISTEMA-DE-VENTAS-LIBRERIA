"""
Router de listas de precios.
Endpoints: GET/POST /price-lists, GET/PUT /price-lists/{id}/items
"""
...
@router.get(
    "/{price_list_id}/items",
    response_model=list[PriceListItemOut],
    dependencies=[Depends(require_role("admin", "cashier"))],
)
async def list_items(price_list_id: int, db: AsyncSession = Depends(get_db)):
    """Lista los ítems de una lista de precios."""
    service = PriceListsService(db)
...
async def replace_items(
    price_list_id: int,
    data: list[PriceListItemCreate],
    db: AsyncSession = Depends(get_db),
):
    """Reemplaza todos los ítems de una lista de precios."""
    service = PriceListsService(db)
